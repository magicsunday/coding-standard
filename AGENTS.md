...
| `bin/check-phpat-subjects.php` | executable (composer `bin`) | the phpat subject-liveness guard — parses a consumer's ArchitectureTest and asserts every rule subject matches a real class (a trait-only namespace subject, the manifested vacuous-rule bug, reds); recognises both of phpat's own rule-discovery paths, the `#[TestRule]` attribute and a public `test*`-named method (GH-58); static, fails closed |
