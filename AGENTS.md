...
| `tests/CheckCheckedExceptionsTest.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | same shape as `CheckDisallowedCallsTest.php` above (both extend `AbstractConsumerPhpstanGateTestCase`; same later-positioned `composer ci:test:checked-exceptions` CI step): proves `phpstan/strict.neon`'s checked-exceptions config (GH-139) fires on an undocumented throws annotation in `tests/consumer/checked-exceptions/CheckedExceptionsFixture.php`, stays quiet on the correctly documented sibling method, and exempts an unchecked programmer-error exception by `LogicException` inheritance (`uncheckedByInheritanceOnly()`, deliberately distinct from the plain-SPL `uncheckedProgrammerError()`, which cannot discriminate that inheritance match on its own) — through the strict tier as installed, not a hand-rebuilt config. Matches on message text (`--error-format=raw` carries no diagnostic identifiers), not on `missingType.checkedException`/`throws.unusedType` directly |
