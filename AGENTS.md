...
| `/.php-cs-fixer.dist.php` | root dev config, `export-ignore`d | this repository's OWN CGL run (`composer ci:test:php:cgl`, GH-83) over `bin/`, `php-cs-fixer/` and `tests/` (`tests/consumer` excluded via a Finder scoped to `tests/` alone — a single Finder with three `in()` roots and one slashless `exclude('consumer')` would instead prune ANY directory named `consumer` under any of the three roots, since Symfony Finder matches a slashless exclude name against the bare basename regardless of which `in()` root it came from). Requires `php-cs-fixer/base.php` directly rather than through the vendor-nested path a consumer uses, same constraint as `/phpstan.neon`'s own row above. Before this gate existed, this package's own first-party PHP was style-checked only incidentally, by the "Consumer smoke" step running against the INSTALLED `tests/consumer` fixture, which never sees `bin/` or `tests/` itself. Proven live by `composer ci:test:php:cgl-cases` (`tests/check-php-cs-fixer-cases.sh`), the same CONTROL/POSITIVE shape as `tests/check-php-analyse-cases.sh` |
