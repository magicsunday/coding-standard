...
| `bin/consumer-checks/helpers.php` | `require`d by `bin/check-consumer-config.php` AND by every sibling `check-*.php` below (top-level `require`, so each sees these as ordinary global function calls — no scope-inheritance trick needed) | shared helpers scoped to this gate rather than `bin/support/`, which holds primitives shared with OTHER scripts: `fail()`, `tooLargeDetail()`, `stripBom()`, `yamlBlock()` (shared by `.phplint.yml` and `deptrac.yaml`), `readBounded()` (wraps `readCapped()` from `bin/support/read-quietly.php`, self-reporting an oversize read — the differently-shaped sibling that row's own note cross-references); NOT a `bin` entry point |
