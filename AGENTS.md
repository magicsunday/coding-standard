...
| `phpstan/strict.neon` | importable | opt-in tier — shipmonk + symplify packs + `disallowed-calls.neon` + extra-strict report params + checked exceptions (GH-139: `exceptions.check.missingCheckedExceptionInThrows` is the only genuinely new flag — `tooWideThrowType` is already PHPStan's own default and deliberately not restated — plus the top-level `checkTooWideThrowTypesInProtectedAndPublicMethods`; see the README section of the same name for the diagnostic-identifier mismatch and the non-final-first-declaration gap) |
...
| `tests/Support/GateResult.php`, `tests/Support/GateProcess.php`, `tests/Support/FixtureDirectory.php` | `require-dev`-only PHPUnit infrastructure, not `bin` entry points | the collaborators `GateTestCase` composes (namespace `MagicSunday\CodingStandard\Test\Support`) — `GateProcess::run()` invokes a gate as a real subprocess (array argv, combined stdout+stderr in best-effort arrival order, an optional working-directory override) and returns a `GateResult` (`output`, `exitCode`, `isDegraded()`); `FixtureDirectory` is the per-test throwaway fixture root (`path()`, `writeJson()`, `cleanup()`); not consumed by any gate/`bin` script directly — used via `GateTestCase` composition (and directly by `CheckDisallowedCallsTest`, the one caller that needs the working-directory override) — and each also has its own dedicated unit-test suite (`GateResultTest.php`, `GateProcessTest.php`, `FixtureDirectoryTest.php`, all extending `PHPUnit\Framework\TestCase` directly, not `GateTestCase`) |
| `tests/CheckDisallowedCallsTest.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | migrated off `tests/check-disallowed-calls-cases.sh` (#81), extending `PHPUnit\Framework\TestCase` directly rather than `GateTestCase`: it verifies `phpstan/disallowed-function-calls.neon`'s own behaviour through the real `phpstan` binary, not one of this package's own `bin/check-*.php` gate scripts, so `GateTestCase`'s accept/reject exit-code contract does not apply. Reuses `GateProcess`/`GateResult` directly (its `run()` takes an optional working-directory argument for exactly this caller) rather than re-invoking `Process` by hand. Self-skips via `setUp()` until `tests/consumer` is installed — see the class's own docblock for why, and `composer ci:test:disallowed-calls`'s description for the CI-step ordering that depends on it |
| `tests/CheckCheckedExceptionsTest.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | same shape as `CheckDisallowedCallsTest.php` above (same self-skip, same `GateProcess`/`GateResult` reuse, same later-positioned `composer ci:test:checked-exceptions` CI step): proves `phpstan/strict.neon`'s checked-exceptions config (GH-139) fires on an undocumented `@throws` in `tests/consumer/checked-exceptions/CheckedExceptionsFixture.php`, stays quiet on the correctly documented sibling method, and exempts an unchecked programmer-error exception (`InvalidArgumentException`, via `LogicException` inheritance) — through the strict tier as installed, not a hand-rebuilt config. Matches on message text (`--error-format=raw` carries no diagnostic identifiers), not on `missingType.checkedException`/`throws.unusedType` directly |
