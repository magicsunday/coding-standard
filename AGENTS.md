...
path — every first-party gate has one. `ci:test:php:lint` and `ci:test:php:analyse`
are third-party checks (a syntax check and a parameterised PHPStan run over the PHP
files under `bin/` and `tests/`) with no first-party logic of their own to drive,
which is why neither is counted here.
