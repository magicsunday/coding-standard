...
| `phpstan/phpat.neon` | importable | opt-in phpat preset (GH-183) — loads `../../../phpat/phpat/extension.neon` by a relative include (valid in Rector's `phpstanConfig` context too, same reason as `base.neon`'s packs) and sets `phpat.show_rule_names`; included NEXT TO `base.neon`, never by it — `phpat/phpat` is a `suggest`, not a `require`. For rules Deptrac cannot express only (structural invariants, sub-layer boundaries); see the *Deptrac first* convention below. The subject-liveness guard `bin/check-phpat-subjects.php` removed by #45 is NOT part of it yet (#184) |
...
- **Deptrac first, phpat only where Deptrac cannot.** Layer dependencies ("who may
  ...
  verified against deptrac 4.7.2: 229 violations). Every phpat rule says which of
  the two it is. Never move phpat back into `base.neon` (#45 took it out;
