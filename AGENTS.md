...
| `phpstan/strict.neon` | importable | opt-in tier — shipmonk + symplify packs + `disallowed-calls.neon` + extra-strict report params + checked exceptions (GH-139: `exceptions.check.missingCheckedExceptionInThrows`/`tooWideThrowType` + the top-level `checkTooWideThrowTypesInProtectedAndPublicMethods` — see the README section of the same name for the diagnostic-identifier mismatch and the non-final-first-declaration gap) |
