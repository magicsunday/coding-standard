...
| `tests/ScrubbedDiagnosticGuardTest.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | a structural regression guard, extending `GateTestCase`, added alongside the #79 migration: walks the `token_get_all()` output of exactly three guarded files (`tests/GateTestCase.php`, `tests/CheckJsConfigsTest.php`, `tests/CheckJsConfigsManifestTest.php` — see its own `guardedFiles()`) looking for a `RISKY_ASSERTIONS`-listed PHPUnit assertion (e.g. `assertSame`/`assertStringContainsString`) whose argument still carries a raw, unscrubbed subprocess-output/PR-editable-content access instead of routing through one of the scrub helpers `GateTestCase` provides (this class's own `SAFE_WRAP_CALLS` lists their names) — the exact class of leak that would forge a CI workflow-command annotation on a genuine test failure. Its own class docblock documents several accepted, non-manifesting detection gaps (curly-brace dynamic access, a bare namespaced function call with no `::`, a double-quoted array-key form) rather than chasing them further, and explicitly defers the same leak shape already present in `tests/CheckCheckedExceptionsTest.php`/`tests/CheckDisallowedCallsTest.php` to a separate tracked issue (#160) instead of widening `guardedFiles()` to them |
