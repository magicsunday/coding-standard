...
against inputs that must produce a finding — including `tests/CheckJsConfigsTest.php`
and `tests/CheckJsConfigsManifestTest.php`, which run the real Biome, `tsc` and jscpd
against the shared configs and the shipped `templates/jscpd.json`. A gate that cannot be
...
| `tests/CheckJsConfigsManifestTest.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | migrated off `tests/check-js-configs.sh`'s `manifest_check()` (#79), extending `GateTestCase`: a repository-hygiene gate defined INLINE in the bash original (no separate `bin/` source, unlike `bin/check-consumer-config.php`/`bin/check-js-config.mjs`), proving this package's OWN `package.json` against its own conventions — the `devEngines.runtime.version` floor, the `engines.node` floor `bin/check-js-config.mjs` needs, the `peerDependencies`-vs-`devDependencies` caret-range lockstep the smoke installs, and `biome/base.json`'s `$schema` URL version against the same pin — via `MANIFEST_CHECK_SCRIPT`, a byte-for-byte copy of the bash original's `node -e '...'` body. It takes a plain directory and reads `package.json` plus `biome/base.json` from it, no packaging pipeline involved, so every case uses an ordinary fresh per-test `fixture()` directory rather than the shared packaged consumer `CheckJsConfigsTest` builds. Needs no installed consumer fixture; `#[Group('js-packaging')]` marks it PHP-version-invariant, so the `build` job's PHPUnit step runs it on only the `php == '8.3'` floor leg, `--exclude-group js-packaging` on the other legs |
| `tests/CheckJsConfigsTest.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | migrated off the rest of `tests/check-js-configs.sh` (#79), extending `GateTestCase`: the packaging-pipeline-dependent half — everything except `manifest_check()` above — packs this package the way npm ships it (`git write-tree` + `git archive` of the committed tree, `.gitattributes` export-ignore applied, then `npm pack --ignore-scripts`), installs the tarball into ONE class-scoped throwaway npm project (`packagedConsumer()`, built lazily once per test run) and drives real Biome/`tsc`/jscpd invocations against it, plus the packaging-adjacent controls (the `files` allow-list vs. the tarball, the `.gitattributes` export-ignore completeness sweep, `--ignore-scripts` enforcement on both `npm pack` and `npm install`, the installed npm bin-entry smoke) and the two remaining self-contained concerns needing no packaging at all (`build_tools_from_devdependencies()`'s own accept/reject fixtures, the README tool-version-pin lockstep). Needs real `git`/`npm`/`tar` and network access for the packaged consumer's own `npm install`, unlike every other suite in this table; `#[Group('js-packaging')]` marks it PHP-version-invariant, so the `build` job's PHPUnit step runs it on only the `php == '8.3'` floor leg, `--exclude-group js-packaging` on the other legs |
...
- **Never put a `"//"` note key in `biome/base.json`.** Biome's config deserializer is
  ...
  parses); only `tests/CheckJsConfigsTest.php`, which loads the config with the real
...
Biome/tsc/jscpd against the shared configs) moved to `tests/CheckJsConfigsTest.php`
and `tests/CheckJsConfigsManifestTest.php`, run by the `build` job's PHPUnit step
instead — both classes carry `#[Group('js-packaging')]`, so that step runs the
