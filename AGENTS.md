...
against inputs that must produce a finding — including the `tests/CheckJsConfigs*Test.php`
suites (built on `tests/Support/AbstractJsConfigsTestCase.php`), which run the real Biome,
`tsc` and jscpd against the shared configs and the shipped `templates/jscpd.json`. A gate that cannot be
...
failure path too: `tests/CheckPhpAnalyseTest.php`, run by `ci:test:phpunit`.
...
| `phpstan/phpat.neon` | importable | opt-in phpat preset (GH-183) — loads `../../../phpat/phpat/extension.neon` by a relative include (valid in Rector's `phpstanConfig` context too, same reason as `base.neon`'s packs) and sets `phpat.show_rule_names`; included NEXT TO `base.neon`, never by it — `phpat/phpat` is a `suggest`, not a `require`. For rules Deptrac cannot express only (structural invariants, sub-layer boundaries, a shared layer narrowed for one module); see the *Deptrac first* convention below. Its subject-liveness guard is the separate `bin/check-phpat-subjects.php` row below (#184) |
...
| `templates/*` | copy-and-adapt | `phpunit.xml.dist`, `infection.json5`, `phplint.yml`, `editorconfig`, `gitattributes`, `jscpd.json` (PHP + JS/TS formats), `deptrac.dist.yaml` (`imports` the shared layers.yaml + declares `paths`), `ArchitectureTest.php` (the phpat rule class for the opt-in preset: `Abstract*` naming + final leaves, plus a commented sub-layer-boundary example — restored from #45's removal, the deprecated `shouldBeFinal()` replaced by `should()->beFinal()`) |
| `/phpunit.xml.dist` | root dev config, `export-ignore`d | this repository's OWN PHPUnit run (`composer ci:test:phpunit`), not a consumer-facing template — three divergences from `templates/phpunit.xml.dist`, re-derive rather than trust this row (`diff templates/phpunit.xml.dist phpunit.xml.dist`): (1) its `bootstrap`/`xsi:noNamespaceSchemaLocation` point at `.build/vendor` because this repo's own `composer.json` overrides `config.vendor-dir` to `.build/vendor` (and `bin-dir` to `.build/bin`); (2) `<source><include>` covers `tests/Support`, `tests/GateTestCase.php` and `bin/support/merge-config-layer.php` instead of `src` — this repo ships no `src/` of its own, only test infrastructure and one shared PHP helper worth coverage-checking — with a matching `<source><exclude>` dropping `tests/Support`'s own `*Test.php` files, which need no `#[CoversClass]` of their own; (3) the testsuite `<exclude>` names `tests/consumer` (its own fixture project, not `tests/Architecture`, which this repo has no equivalent of — nothing here defines PHPStan rule classes to exclude from a PHPUnit run). Running the shipped consumer-template gate against this repo's own root config therefore correctly reports a `<source><include>` violation (it expects `src`) — this row explains why that divergence is intentional, not a defect to "fix" |
| `/phpstan.neon` | root dev config, `export-ignore`d | this repository's OWN PHPStan run (`composer ci:test:php:analyse`) over the PHP files under `bin/` and `tests/` (`tests/consumer` excluded — its own fixture project, covered by the "Consumer smoke" CI steps instead), level 6 plus `phpstan/disallowed-function-calls.neon`'s case-folding bans (see that row — not a restated copy); cannot `includes: - phpstan/disallowed-calls.neon` the way a consumer does, see the file's own comment for why. Its own include of the ban list is proven live by `tests/CheckPhpAnalyseTest.php` (run by `composer ci:test:phpunit`; migrated off `tests/check-php-analyse-cases.sh`, #71) — a link `CheckDisallowedCallsTest` cannot cover, since that one only ever runs through `tests/consumer`'s installed copy |
| `/.php-cs-fixer.dist.php` | root dev config, `export-ignore`d | this repository's OWN CGL run (`composer ci:test:php:cgl`, GH-83) over `bin/`, `php-cs-fixer/` and `tests/` (`tests/consumer` excluded). A single Finder with three `in()` roots and one slashless `exclude('consumer')` would prune ANY directory named `consumer` under any of the three roots (Symfony Finder matches a slashless exclude name against the bare basename regardless of which `in()` root it came from), so `tests/` gets its own dedicated Finder appended into the main one — but a bare `exclude('consumer')` on even that scoped Finder still matches any nested `consumer`-named directory under `tests/`, not only the top-level fixture, so it uses an anchored `notPath('#^consumer(/|$)#')` instead (matched against the path relative to `tests/`). Requires `php-cs-fixer/base.php` directly rather than through the vendor-nested path a consumer uses, same constraint as `/phpstan.neon`'s own row above. Before this gate existed, this package's own first-party PHP was style-checked only incidentally, by the "Consumer smoke" step running against the INSTALLED `tests/consumer` fixture, which never sees `bin/` or `tests/` itself. Proven live by `tests/CheckPhpCsFixerTest.php` (run by `composer ci:test:phpunit`; migrated off `tests/check-php-cs-fixer-cases.sh`, #71), which drives the real Finder (no explicit path override) against throwaway probes under `bin/consumer/`, `tests/Support/consumer/` and `tests/consumer/` to prove the scoping itself, alongside a CONTROL/POSITIVE pair mirroring `tests/CheckPhpAnalyseTest.php` |
| `biome/base.json`, `tsconfig/base.json` | importable (`extends`) | the JS/TS repos |
| `bin/check-consumer-config.php` | executable (composer `bin`) | the template lockstep gate's thin orchestrator (#48; split once the file crossed 1000 lines) — defines `MAX_JSONC_BYTES`/`MAX_TEXT_BYTES`, resolves `$repoRoot`/`$packageRoot`, `require_once`s the shared support files and every `bin/consumer-checks/check-*.php` below, calls each `checkXxx($violations, …)` in turn, then prints the final report. Asserts each consumer copy's stable region (strict phpunit flags, jscpd/phplint/editorconfig invariants, the `deptrac.yaml` shared import, uniform `src`/`tests`), ignores per-repo paths; also covers `biome.json`/`biome.jsonc`/`tsconfig.json` on the narrower extends-stub contract, keyed on the consumer declaring the npm dependency (the `"//"` guard, an unopenable Biome/TypeScript config, one past the size cap and a broken `package.json` probe do not wait for adoption — the probe itself runs only where a `biome.json`, `biome.jsonc` or `tsconfig.json` exists), parsed as JSONC |
| `bin/consumer-checks/helpers.php` | `require`d by `bin/check-consumer-config.php` alone, before any sibling `check-*.php` below is required or called — none of those six require it themselves, they just call its functions as ordinary already-declared globals (PHP functions are global regardless of which file declared them, so no scope-inheritance trick is needed) | shared helpers scoped to this gate rather than `bin/support/`, which holds primitives shared with OTHER scripts: `fail()`, `tooLargeDetail()`, `stripBom()`, `yamlBlock()` (shared by `.phplint.yml` and `deptrac.yaml`), `readBounded()` (wraps `readCapped()` from `bin/support/read-quietly.php`, self-reporting an oversize read — the differently-shaped sibling that row's own note cross-references); NOT a `bin` entry point |
| `bin/consumer-checks/check-phpunit-xml.php` | `require`d by `bin/check-consumer-config.php` | `checkPhpunitXml($violations, $repoRoot)` — the REQUIRED phpunit.xml contract: the strict-flag set (`$requiredRootFlags`) plus the uniform `src`/`tests` layout; NOT a `bin` entry point |
| `bin/consumer-checks/check-jscpd-json.php` | `require`d by `bin/check-consumer-config.php` | `checkJscpdJson($violations, $repoRoot)` — the optional `.jscpd.json` contract: the zero-tolerance thresholds and the `$extensionSpellings` deny-list; NOT a `bin` entry point |
| `bin/consumer-checks/check-phplint-yml.php` | `require`d by `bin/check-consumer-config.php` | `checkPhplintYml($violations, $repoRoot)` — the optional `.phplint.yml` contract: the `extensions:` block must list `- php`; NOT a `bin` entry point |
| `bin/consumer-checks/check-editorconfig.php` | `require`d by `bin/check-consumer-config.php` | `checkEditorconfig($violations, $repoRoot)` — the optional `.editorconfig` contract: the 4-space house indent plus the Makefile tab override; NOT a `bin` entry point |
| `bin/consumer-checks/check-deptrac-yaml.php` | `require`d by `bin/check-consumer-config.php` | `checkDeptracYaml($violations, $repoRoot)` — the optional `deptrac.yaml` contract: the shared layer ruleset must be imported; NOT a `bin` entry point |
| `bin/consumer-checks/check-biome-tsconfig.php` | `require`d by `bin/check-consumer-config.php` | `checkBiomeTsconfig($violations, $repoRoot, $packageRoot)` — by far the largest split (re-derive the current size rather than trusting a number here: `wc -l bin/consumer-checks/check-biome-tsconfig.php`; the biome.json/tsconfig.json contract was already past 1000 lines as one section before the split), covering the optional `biome.json`/`biome.jsonc`/`tsconfig.json` `extends`-stub contract: the JSONC decode pipeline (`$stripJsonc`, `$loadJsonc`), the `extends`-chain resolution (GH-36: `$resolveExtendsLayers`, `$foldExtendsChain`), the adoption gate (`$npmDependencyDeclared`), and the per-rule/per-flag walk (`$sharedBiomeRules`, `$pinnedFlags`). These closures stay LOCAL to this one file — unlike `helpers.php`'s functions, nothing outside this file calls them; NOT a `bin` entry point |
| `bin/check-phpat-subjects.php` | executable (composer `bin`) | the phpat subject-liveness guard (#184, restored from `da192b4^` after #45 removed it) — for a repository adopting the opt-in `phpstan/phpat.neon` preset only, wired as its `ci:test:php:phpat-subjects` script (`["check-phpat-subjects.php ."]`), rolled out script-first like `ci:test:php:templates`. Parses the consumer's `ArchitectureTest` and asserts every rule subject matches a real class in `src/` (a trait-only namespace subject, the manifested vacuous-rule bug, reds); recognises both of phpat's rule-discovery paths, the `#[TestRule]` attribute under any import alias/casing and a public `test*`-named method (GH-58); `isAbstract()` is not liveness-checked; static (never runs PHPStan), fails closed, reads through `readCapped()` and reports through `safeReportValue()`. Reads subjects only, so it does not depend on phpat's builder API (`should()->beFinal()` vs. the deprecated `shouldBeFinal()`) |
| `bin/check-js-config.mjs` | executable (npm `bin`, `check-js-config`) | the node-only front end for the SAME `biome.json`/`biome.jsonc`/`tsconfig.json` contract `bin/consumer-checks/check-biome-tsconfig.php` covers — for a repository with no `composer.json` at all (#32). Not a reimplementation kept in step by hand: every biome/tsconfig case in the `tests/CheckConsumerConfig*Test.php` classes also runs this gate against the identical fixture directory and requires the identical verdict (the `assertBoth*()` helpers of their shared base, `tests/Support/AbstractConsumerConfigTestCase.php`), so the two contracts cannot silently drift apart |
| `bin/support/bounded-reader.mjs` | `import`ed by `bin/check-js-config.mjs` and `bin/support/jsonc.mjs` (`grep -rln "from '\./support/bounded-reader.mjs'\|from '\./bounded-reader.mjs'" bin`) | `tooLargeDetail()`, `isFile()`, `isDirectory()`, `readBoundedBytes()` — extracted out of `bin/check-js-config.mjs` (#74) once that file crossed 1000 lines. Self-reporting stays with the caller: `readBounded()` (which calls `fail()`/`violations`) was deliberately NOT moved here, mirroring `bin/consumer-checks/helpers.php`'s own `readBounded()` function staying out of `bin/support/read-quietly.php` for the same reason (see that row's own note); NOT a `bin` entry point, so it needs no `"bin"` row in package.json but must stay inside the npm tarball |
| `bin/support/jsonc.mjs` | `import`ed by `bin/check-js-config.mjs` (`grep -rln "from '\./support/jsonc.mjs'" bin`) | the byte-level JSONC-to-JSON decode pipeline mirroring PHP's `json_decode()` (BOM strip, comment/trailing-comma strip, depth cap, strict UTF-8, lone-surrogate rejection) — extracted out of `bin/check-js-config.mjs` (#74) once that file crossed 1000 lines. Exports only `stripBomBytes()`, `decodeJsonLikePhp()` and `loadJsonc(path, maxBytes)` — the three call sites `bin/check-js-config.mjs` uses directly; the rest of the pipeline stays module-private, called only internally. `bin/consumer-checks/check-biome-tsconfig.php` carries the equivalent logic inline (`$stripJsonc`, `$loadJsonc`, …) rather than in its own `bin/support/*.php` file, so there is no PHP-side counterpart to cross-reference the way `merge-config-layer.php`/`.mjs` do; NOT a `bin` entry point, so it needs no `"bin"` row in package.json but must stay inside the npm tarball |
| `bin/support/safe-report-value.php`, `bin/support/safe-report-value.mjs` | `require`d / `import`ed by the gates that echo a value read out of a repository file (`grep -rln "^require_once .*safe-report-value" bin tests` for the PHP requirers — anchored on the statement, since the bare path also matches files that only mention it — and `grep -rl "from '\./support/safe-report-value.mjs'" bin` for the one node requirer, `bin/check-js-config.mjs`) | scrubs C0/DEL and breaks the legacy `##[` workflow-command prefix on any such value, and caps it at 64 bytes — the `bin/` gates run in the consumer's CI over pull-request content and this repository's own gate over its own, and the runner scans both STDERR and STDOUT for workflow commands; NOT a `bin` entry point, so it needs no `"bin"` row in composer.json/package.json but must stay inside the dist archive/npm tarball. The PHP file additionally exports `scrubReportControlBytes()`, the uncapped control-byte-strip-plus-`##[`-break core `safeReportValue()` itself wraps with its 64-byte cap — also called directly by `scrubbedForDiagnostic()` in `tests/Support/ScrubbedDiagnostics.php` (used by `tests/GateTestCase.php` and `tests/Support/AbstractConsumerPhpstanGateTestCase.php`, so inherited by every suite extending either, the `tests/CheckJsConfigs*Test.php` suites included), which layers a further `::`-breaking step on top for its own uncapped developer-diagnostic messages. This is a PHP/JS asymmetry: the `.mjs` sibling was not split the same way and still only exposes the combined, capped `safeReportValue()` |
...
| `tests/GateTestCase.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | abstract base class (`MagicSunday\CodingStandard\Test\GateTestCase`) for suites migrated off `tests/harness.sh` (#71) — the five `assertGateAccepts`/`assertGateRejects`/`assertGateUsageError`/`assertGateReportIsInert`/`assertGateReportsOnce` decisions (`assertGateReportIsInert()` takes an appended, defaulted expected exit code — the drift verdict, 1, unless a caller declares otherwise — ported from `harness_report_is_inert`'s 5th argument for a gate whose forge-prone value is refused at exit 2) plus `fixture()` and `root()` (this repository's own checkout root), ported from `harness.sh`'s `harness_decide_*` functions; consumed by its own meta-suite `tests/GateTestCaseTest.php` (`grep -c '#\[Test\]' tests/GateTestCaseTest.php`) today, `tests/CheckVersionLockstepTest.php` (#80), the `tests/CheckConsumerConfig*Test.php` classes (#78, via `tests/Support/AbstractConsumerConfigTestCase.php` since GH-52), `tests/CheckJsConfigsManifestTest.php` and `tests/Support/AbstractJsConfigsTestCase.php` (#79, split into four suites by #75), `tests/CheckReleaseTagLockstepTest.php` (#71), the `tests/CheckPhpatSubjects*Test.php` classes (#184, via `tests/Support/AbstractPhpatSubjectsTestCase.php`), and the base for every remaining suite in #82 that runs one of this package's OWN gate scripts with the accept/reject/usage-error exit-code contract (see that issue for current migration status; #81 is the one exception — see the `tests/CheckDisallowedCallsTest.php` row below). Deliberately exempt from the `Abstract*` naming rule (see the class's own docblock): it plays the same role as PHPUnit's own `TestCase`, itself abstract and unprefixed — every future suite extends it the same way it extends `TestCase`. Also gets `scrubbedForDiagnostic()` through the `ScrubbedDiagnostics` trait (its own row below) rather than declaring it, and every concrete subclass (the `tests/CheckJsConfigs*Test.php` suites and `tests/CheckJsConfigsManifestTest.php`'s own `assertManifestRejects()`/`assertManifestReportsValue()` included) reuses it rather than growing its own copy: `assertGateReportIsInert()`, `assertReportCarries()` and `runAndAssertVerdict()` (the shared precondition every `assertGate*()` decision calls first) fail via a manual condition + `self::fail()` rather than PHPUnit's own string-containment/regex assertions, because `$result->output` is exactly the value a poisoned fixture (e.g. a `##[error]forged` devDependency name) can carry, and PHPUnit's own `Constraint::fail()`/`failureDescription()` mechanism unconditionally re-embeds the full, raw haystack into a failed assertion's own message — a real failure there would forge, in PHPUnit's own failure output, the very CI workflow-command annotation these checks exist to prove is prevented. The trait's other message helpers compose `scrubbedForDiagnostic()` into `self::fail()`-ready messages and are among the most heavily reused members of the suite (`grep -rc '::diagnosticMessage(' tests/*.php tests/Support/*.php` re-derives the current call count rather than trusting a number frozen here): `diagnosticMessage(label, output)` always composes `label . "\n" . scrubbedForDiagnostic(output)`; `messageOrDefault(message, default, output)` returns `message` verbatim with no scrub applied when non-empty, otherwise falls back to `diagnosticMessage(default, output)`; `messageWithOutput(message, default, output)` picks `message` when non-empty else `default` the same way, but — unlike `messageOrDefault()` — always appends the scrubbed `output` regardless, by delegating to `diagnosticMessage(message !== '' ? message : default, output)`. Also exposes `protected static assertThrows(callable $invoke, string $exceptionClass, string $rejectedMessage): Throwable`, collapsing the repeated `try { $invoke(); } catch (...) { $thrown = $exception; } self::assertNotNull($thrown, ...)` shape used across the suite (`grep -rn "self::assert[T]hrows(" tests/` re-derives the current call sites rather than trusting a number frozen here) into one call; it rethrows a caught exception that is not an instance of `$exceptionClass` uncaught rather than swallowing a wrong exception type as "not thrown" |
| `tests/Support/GateResult.php`, `tests/Support/GateProcess.php`, `tests/Support/FixtureDirectory.php` | `require-dev`-only PHPUnit infrastructure, not `bin` entry points | the collaborators `GateTestCase` composes (namespace `MagicSunday\CodingStandard\Test\Support`) — `GateProcess::run()` invokes a gate as a real subprocess (array argv, combined stdout+stderr in best-effort arrival order, an optional working-directory override) and returns a `GateResult` (`output`, `exitCode`, `isDegraded()`); `GateProcess::runRaw()` is the generalised body `run()` itself delegates to, for a caller whose invocation does not fit `run()`'s fixed `<command...> <fixtureDir>` argv shape (no fixture-directory argument appended, plus an optional `$env` merged onto the inherited environment and an optional `$timeout`) — its own real second consumer, besides `run()`, is `tests/Support/AbstractJsConfigsTestCase.php`'s `runCommand()`, for the arbitrary git/npm/tar/biome/tsc/jscpd invocations of the suites extending it, and `tests/CheckJsConfigsManifestTest.php`'s `ROOT`-environment-variable invocation of `manifest_check()`; `FixtureDirectory` is the per-test throwaway fixture root (`path()`, `writeJson()`, `cleanup()`); not consumed by any gate/`bin` script directly — used via `GateTestCase` composition and, since GH-139, via `AbstractConsumerPhpstanGateTestCase::runPhpstan()` (its `run()`'s optional working-directory argument is what that method needs) — and each also has its own dedicated unit-test suite (`GateResultTest.php`, `GateProcessTest.php`, `FixtureDirectoryTest.php`, all extending `PHPUnit\Framework\TestCase` directly, not `GateTestCase`) |
| `tests/Support/AbstractConsumerPhpstanGateTestCase.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | shared base (GH-139) for a self-test that proves a shared PHPStan config fires by running the real, installed `tests/consumer` `phpstan` binary against a fixture — extracted out of `CheckDisallowedCallsTest.php` once `CheckCheckedExceptionsTest.php` needed the identical shape (`root()`/`consumer()`/`phpstanBinary()`/`runPhpstan(string $config, string $fixturePath)`/the self-skip `setUp()`/`assertResultIsNotDegraded()`). Not `GateTestCase`: that class's accept/reject/usage-error exit-code contract is for this package's OWN `bin/check-*.php` gate scripts, not a third-party binary's PHPStan findings — both consumers extend this class directly, not `TestCase`. What the two lineages DO share is the failure-message hygiene, through the `ScrubbedDiagnostics` trait: a PHPStan report is built from fixture content, so both consumers assert on it via `assertOutputContains()`/`assertOutputDoesNotContain()` and build any message that carries the report with `diagnosticMessage()`, never a raw `$result->output` (GH-160) |
| `tests/Support/ScrubbedDiagnostics.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | the failure-message half of a gate suite's PR-content hygiene, shared by the two test-case lineages that assert on a subprocess report — `GateTestCase` and `AbstractConsumerPhpstanGateTestCase` (`grep -rn "^    use ScrubbedDiagnostics;" tests/` re-derives the users). Holds the message helpers `scrubbedForDiagnostic()`/`diagnosticMessage()`/`messageOrDefault()`/`messageWithOutput()` and the two containment assertions `assertOutputContains()`/`assertOutputDoesNotContain()`, each a manual `str_contains()` + `self::fail()` because PHPUnit's own string-containment constraints re-embed the whole raw haystack into the exception message. Requires `bin/support/safe-report-value.php` itself. Covered through `#[CoversTrait]` on `tests/GateTestCaseTest.php`, which is where the failure-message behaviour of both assertions is proven with a poisoned report |
...
| `tests/CheckPhpatPresetTest.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | same shape as `CheckCheckedExceptionsTest.php` above (extends `AbstractConsumerPhpstanGateTestCase`; later-positioned `composer ci:test:phpat-preset` CI step, in both the `build` and `prefer-lowest` jobs): proves `phpstan/phpat.neon` (GH-183) through `tests/consumer/phpstan-phpat.neon` — the rule class `tests/consumer/phpat/PhpatFixtureRules.php` (autoloaded via the fixture's `autoload-dev`, as a real consumer's rule class is) reports the non-final `Leaf\OpenLeaf` under its rule name (the `show_rule_names` prefix), stays quiet on `Leaf\FinalLeaf` and reports nothing else under `phpat/`; and, through `tests/consumer/phpstan-phpat-base-only.neon` (the same rule class registered against `base.neon` WITHOUT the preset), that `base.neon` never loads phpat. The registration in that contrast config is what makes it discriminate — without it no rule runs whether phpat is loaded or not |
| `tests/Support/AbstractPhpatSubjectsTestCase.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | abstract base (namespace `MagicSunday\CodingStandard\Test\Support`, extending `GateTestCase`) of the `bin/check-phpat-subjects.php` suite, ported from the bash `tests/check-phpat-subjects-cases.sh` #45 removed (#184): the gate command (plus `gateFromInside()`, which runs it against `.` so a poisoned file NAME survives `safeReportValue()`'s 64-byte cap), byte-for-byte ports of the bash fixture writers and the shared rule-method bodies. Its class docblock maps each part of the gate to the class a new case belongs in |
| `tests/CheckPhpatSubjectsTest.php`, `tests/CheckPhpatSubjectsTokenWalkTest.php`, `tests/CheckPhpatSubjectsReportTest.php`, `tests/CheckPhpatSubjectsDiscoveryTest.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | one `final` class per section seam of the bash original, each extending `AbstractPhpatSubjectsTestCase`: the liveness arms, fail-closed reports, read arms and usage errors (…`Test`, also the shipped `templates/ArchitectureTest.php` accepted/rejected as copied); the token walk, class inventory, `NAMESPACE_ROOT` resolution and linear-time cases (…`TokenWalkTest`); the report-is-inert cases (…`ReportTest`); the `test*`-name discovery path, visibility/nesting and import-alias tracking (…`DiscoveryTest`). Every bash case is ported, the import-alias cluster as two data providers (so `grep -c '#\[Test\]'` counts methods, not cases); the three mode-`000` cases self-skip under uid 0. Need no installed consumer fixture and run as part of the plain `composer ci:test:phpunit` step |
| `tests/CheckPhpAnalyseTest.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | migrated off `tests/check-php-analyse-cases.sh` (#71), extending `GateTestCase` for its fixture directory and scrubbed-message helpers: a CONTROL/POSITIVE pair running this repository's OWN `.build/bin/phpstan` against the root `/phpstan.neon` on a throwaway probe outside `paths`, proving the config's include of `phpstan/disallowed-function-calls.neon` fires (`Calling strtolower() is forbidden`) and that a clean probe stays clean. Needs no installed consumer fixture and runs as part of the plain `composer ci:test:phpunit` step; a missing root phpstan binary fails rather than skips, since that step runs after `composer install` |
| `tests/CheckPhpCsFixerTest.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | migrated off `tests/check-php-cs-fixer-cases.sh` (#71), extending `GateTestCase`: a CONTROL/POSITIVE pair running this repository's OWN `.build/bin/php-cs-fixer` against the root `/.php-cs-fixer.dist.php` on a throwaway probe, a FINDER SCOPE case driving the real Finder (no explicit path) over uniquely named probes under `bin/consumer/`, `tests/Support/consumer/` and `tests/consumer/`, and four PRESERVATION cases pinning the ownership contract of its `mkdirOwned()`/`rmdirIfOwned()` cleanup (a probe directory is removed only when this run created it and it is still empty, and a refused rmdir raises nothing). Needs no installed consumer fixture and runs as part of the plain `composer ci:test:phpunit` step |
...
| `tests/LintJsonTest.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | migrated off `tests/lint-json-cases.sh` (#71), extending `GateTestCase`: it drives `tests/lint-json.php` (`composer ci:test:json`) itself, so the accept/reject/report-is-inert exit-code contract applies directly — the vacuity guard reached both ways (no `*.json` at all, every discovered file excluded), `INVALID`/`MISSING`/`UNREADABLE`/`TOO LARGE`/`Could not scan`/`Not a directory` each driven, the `PRUNED_DIRECTORY_NAMES` entries one data-provider case apiece, both `EXCLUDED_JSON_FILES` entries, a live leaf symlink refused and a symlinked directory never descended into, and both `MAX_SCAN_DEPTH`/`MAX_JSON_LINT_BYTES` bounds pinned on both sides of the limit. Carries one decision `GateTestCase` does not, `assertGateAcceptsWithInertReport()`: this gate echoes a PR-controlled file name on its ACCEPT path too, which `assertGateReportIsInert()` (reject-only) cannot cover. The three mode-`000` cases self-skip under uid 0, the same as the `CheckConsumerConfig*Test` classes'. Needs no installed consumer fixture and runs as part of the plain `composer ci:test:phpunit` step |
| `tests/CheckGitattributesLockstepTest.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | migrated off `tests/check-gitattributes-lockstep-cases.sh` (#71), extending `GateTestCase`: it drives `tests/check-gitattributes-lockstep.php` (`composer ci:test:gitattributes`, GH-38) itself, so the accept/reject/usage-error/report-is-inert exit-code contract applies directly. Each case builds its repository under a `case/` subdirectory of the fixture directory, leaving the fixture root as the OUTSIDE location its `..`-traversal cases point at. Its at-the-size-cap fixtures use `GateTestCase::padTextToCap()`, moved there from `tests/CheckVersionLockstepTest.php`'s private copy once this became the second caller. Needs no installed consumer fixture and runs as part of the plain `composer ci:test:phpunit` step |
| `tests/CheckReleaseTagLockstepTest.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | migrated off `tests/check-release-tag-lockstep-cases.sh` (#71), extending `GateTestCase`: it drives `tests/check-release-tag-lockstep.php` (GH-42) itself, so the accept/reject/usage-error/report-is-inert exit-code contract applies directly. Each case builds real, disposable local git repositories under its own fixture directory — a bare origin, a working clone, and where needed an unrelated or deliberately corrupted seed history — and hands the gate its remote through an `env CHECK_RELEASE_TAG_REMOTE=…` command prefix (`env -u` for the default-`origin` cases), never the real network. Needs no installed consumer fixture and runs as part of the plain `composer ci:test:phpunit` step — on every pull_request, unlike the gate itself (`composer ci:test:release-tag`), which only runs on a push to `main` or a tag push |
...
| `tests/Support/AbstractConsumerConfigTestCase.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | the shared base (GH-52, #75) of the consumer-config suite — migrated off `tests/check-consumer-config-cases.sh` (#78) as one class, split per contract once it crossed 4000 lines so the test layout mirrors the gate's own `bin/consumer-checks/check-*.php` split (#48). Abstract, extends `GateTestCase`, namespace `MagicSunday\CodingStandard\Test\Support` (the `AbstractConsumerPhpstanGateTestCase` precedent). Holds everything the per-contract classes below share: the gate paths (`phpGate()`/`nodeGate()`/`canon()`), the shared fixture builders (`mkCase()`/`mkJsCase()`/…), the `MAX_JSONC_BYTES`/`MAX_TEXT_BYTES` mirrors, the gate-source extractors (`gateSource()`/`extractQuotedList()`) and — the one thing that must never be copied into a subclass — the `assertBoth*()` helpers that run BOTH `bin/check-consumer-config.php` and `bin/check-js-config.mjs` against the same fixture with the same expected verdict: the only DIFFERENTIAL suite, so the differential property keeps a single definition however many classes carry cases (#75). A helper or constant only one contract uses lives in that contract's class instead. Its class docblock maps each `check-*.php` file to the class a new case belongs in |
| `tests/CheckConsumerConfigTest.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | the orchestrator-level cases of `bin/check-consumer-config.php`, extending `AbstractConsumerConfigTestCase`: what belongs to no single contract — the canon fixture, the full shipped template set, the plain-text `MAX_TEXT_BYTES` cap and unreadable-file handling every plain-text reader shares through `helpers.php`'s `readBounded()`, and the not-a-directory usage error. Every class in this group, base included, needs no installed consumer fixture and runs as part of the plain `composer ci:test:phpunit` step, the same as `CheckVersionLockstepTest` |
| `tests/CheckConsumerConfigPhpunitXmlTest.php`, `tests/CheckConsumerConfigJscpdJsonTest.php`, `tests/CheckConsumerConfigPhplintYmlTest.php`, `tests/CheckConsumerConfigEditorconfigTest.php`, `tests/CheckConsumerConfigDeptracYamlTest.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | one `final` class per PHP-only contract, each extending `AbstractConsumerConfigTestCase` and named after the `bin/consumer-checks/check-*.php` file it drives (the PHP gate alone — `bin/check-js-config.mjs` has no counterpart for `phpunit.xml`, `.jscpd.json`, `.phplint.yml`, `.editorconfig` or `deptrac.yaml`). The `$requiredRootFlags`/`$extensionSpellings` lockstep tables are each read from the split that declares them (mirroring `CheckDisallowedCallsTest`'s banned-function extraction) rather than hand-copied, and proven in both directions against an independent list the class drives a case for |
| `tests/CheckConsumerConfigBiomeTsconfigTest.php`, `tests/CheckConsumerConfigBiomeTsconfigExtendsChainTest.php`, `tests/CheckConsumerConfigBiomeTsconfigJsoncTest.php`, `tests/CheckConsumerConfigBiomeTsconfigAdoptionTest.php`, `tests/CheckConsumerConfigBiomeTsconfigPinnedFlagsTest.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | the `bin/consumer-checks/check-biome-tsconfig.php` contract, split further along that file's own seams because it alone was past 2000 lines of cases: the `extends` specifier and effective-config toggle/per-language walk (…`Test`), the GH-36 extends-chain fold (…`ExtendsChainTest`), the JSONC decode pipeline with its size caps and report scrubbing (…`JsoncTest`), the npm-dependency adoption gate and the checks that do not wait for it (…`AdoptionTest`), and the `$pinnedFlags` table derived from `tsconfig/base.json` (…`PinnedFlagsTest`). Every case runs BOTH gates through the base's `assertBoth*()` helpers |
| `tests/CheckJsConfigsManifestTest.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | migrated off `tests/check-js-configs.sh`'s `manifest_check()` (#79), extending `GateTestCase`: a repository-hygiene gate defined INLINE in the bash original (no separate `bin/` source, unlike `bin/check-consumer-config.php`/`bin/check-js-config.mjs`), proving this package's OWN `package.json` against its own conventions — the `devEngines.runtime.version` floor, the `engines.node` floor `bin/check-js-config.mjs` needs, the `peerDependencies`-vs-`devDependencies` caret-range lockstep the smoke installs, and `biome/base.json`'s `$schema` URL version against the same pin — via `MANIFEST_CHECK_SCRIPT`, a byte-for-byte copy of the bash original's `node -e '...'` body. It takes a plain directory and reads `package.json` plus `biome/base.json` from it, no packaging pipeline involved, so every case uses an ordinary fresh per-test `fixture()` directory rather than the shared packaged consumer `AbstractJsConfigsTestCase` builds. Needs no installed consumer fixture; `#[Group('js-packaging')]` marks it PHP-version-invariant, so the `build` job's PHPUnit step runs it on only the `php == '8.3'` floor leg, `--exclude-group js-packaging` on the other legs |
| `tests/Support/AbstractJsConfigsTestCase.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | abstract base (namespace `MagicSunday\CodingStandard\Test\Support`, extending `GateTestCase`) for the four suites split out of the former single `tests/CheckJsConfigsTest.php` (#75/#52, along that file's own section banners), which was migrated off the rest of `tests/check-js-configs.sh` (#79) — everything except `manifest_check()`. Holds what the four share: `packagedConsumer()`, which packs this package the way npm ships it (`git write-tree` + `git archive` of the committed tree, `.gitattributes` export-ignore applied, then `npm pack --ignore-scripts`) and installs the tarball into ONE throwaway npm project, plus `runCommand()`, `mutateConsumerFile()` (with the `tearDown()` that restores every mutation), `makeTempDir()`, the `requirePackedTarball()`/`requireSuccessfulInit()`/`requireSuccessfulInstall()` throw sites, `buildToolsFromDevDependencies()` (`BUILD_TOOLS_SCRIPT`), `archiveIndexInto()`/`pathsMissingFromArchive()` and `assertMessageDoesNotForgeWorkflowCommand()`. The `packagedConsumer()` cache is a static property declared HERE ONLY, so every subclass shares one storage and the expensive `npm install` runs once per PHPUnit process, whichever suite asks first and in whatever order they run; for the same reason its directories are removed by a `register_shutdown_function()` registered on the first build, not by a `tearDownAfterClass()`, which fires once per subclass and would force every later suite to reinstall. Needs real `git`/`npm`/`tar` and network access for that `npm install`, unlike every suite outside this family |
| `tests/CheckJsConfigsConsumerSmokeTest.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | extends `AbstractJsConfigsTestCase`: the packaged-consumer smoke — real Biome/`tsc`/jscpd against the shared configs as installed from the tarball: the accept smoke, the controls proving the shared rules actually bite (each asserting the diagnostic, not the bare exit status), the `extensionMappings`/jscpd format-name bijections and `templates/jscpd.json`'s format names against jscpd itself. `#[Group('js-packaging')]`, like every suite in this family |
| `tests/CheckJsConfigsPackagingTest.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | extends `AbstractJsConfigsTestCase`: the packaging controls — `--ignore-scripts` enforcement on both `npm pack` and `npm install`, the installed npm bin-entry smoke (`check-js-config`), the `files` allow-list vs. the tarball, and the `.gitattributes` export-ignore completeness sweep with its `archiveIndexInto()`/`pathsMissingFromArchive()` failure-path fixtures (#100). `#[Group('js-packaging')]` |
| `tests/CheckJsConfigsToolPinsTest.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | extends `AbstractJsConfigsTestCase`: the two concerns needing no packaging at all — `build_tools_from_devdependencies()`'s own accept/reject fixtures and the README tool-version-pin lockstep. Never calls `packagedConsumer()`, but keeps `#[Group('js-packaging')]` with the rest of the family (it still drives `node`) |
| `tests/CheckJsConfigsHarnessTest.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | extends `AbstractJsConfigsTestCase`: the regression tests for that base's own harness — `tearDown()`'s restore-failure handling and cache invalidation (which reinstates the intact consumer afterwards, so the invalidation it proves does not cost a second install), `makeTempDir()`'s mkdir()-failure branch, the scrubbing at the real npm pack/init/install throw sites, `assertMessageDoesNotForgeWorkflowCommand()`'s true-positive branch, and the shared `scrubReportControlBytes()`/`scrubbedForDiagnostic()` scrub core driven directly. `#[Group('js-packaging')]` — `.github/workflows/ci.yml`'s `build` job runs that group on only the `php == '8.3'` floor leg, `--exclude-group js-packaging` on the other legs |
| `tests/ScrubbedDiagnosticGuardTest.php` | `require-dev`-only PHPUnit infrastructure, not a `bin` entry point | a structural regression guard, extending `GateTestCase`, added alongside the #79 migration: walks the `token_get_all()` output of the files its own `guardedFiles()` lists looking for any `assert*()` call or `fail()` (`GUARDED_CALL_PATTERN`, a pattern rather than a hand-kept name list since #164) whose argument still carries a raw, unscrubbed subprocess-output/PR-editable-content access instead of routing through one of the `ScrubbedDiagnostics` scrub helpers (this class's own `SAFE_WRAP_CALLS` maps each name to the one argument it scrubs, so a wrap's label/message arguments stay in the scan) — the exact class of leak that would forge a CI workflow-command annotation on a genuine test failure. Its own class docblock documents several accepted, non-manifesting detection gaps (curly-brace dynamic access, a bare namespaced function call with no `::`, a double-quoted array-key form) rather than chasing them further. The `tests/CheckConsumerConfig*Test.php` classes and their base are deliberately not listed: what they must keep out of a failure message is a repository file's content held in a local variable, a shape the guard's pattern cannot see, so its one live instance is pinned by a regression test of its own instead (`CheckConsumerConfigPhpunitXmlTest::aFailedCanonFlagCheckDoesNotEmbedTheCanonContentInItsMessage()`) |
...
- **JS/TS:** a GitHub **git dependency** — `github:magicsunday/coding-standard#<tag>`
  ...
  **Node tool versions track the current major — always pin forward.** The peer
  ...
  `tests/CheckJsConfigsManifestTest.php` asserts each against the exact pin it installs;
  moving one means bumping the pin and the range in the SAME edit, then letting
  `tests/CheckJsConfigsManifestTest.php` vet both — the gate is a lockstep check BETWEEN the two
...
- **Deptrac first, phpat only where Deptrac cannot.** Layer dependencies ("who may
  depend on whom") are Deptrac's (`deptrac/layers.yaml`). phpat, through the opt-in
  `phpstan/phpat.neon`, is for the two rule kinds Deptrac cannot express: structural
  invariants on a class (a modifier, a name, a required interface — Deptrac can
  forbid a dependency but not require one or inspect a modifier) and sub-layer
  boundaries (Deptrac checks a class against every layer it belongs to, so one
  sub-namespace of a shared layer cannot be granted an edge the rest is denied —
  verified against deptrac 4.7.2: 229 violations). A third kind follows from how
  Deptrac merges imports: it UNITES the rulesets, so a consumer's `deptrac.yaml` can
  add layers and widen a shared one but never narrow a shared layer for one module
  (verified against deptrac 4.7.2: a local `Model: [Module]` silences a
  `Model`→`Module` violation the shared ruleset reports) — such a per-module
  narrowing is a phpat rule too. Every phpat rule says which of the three it is. Never move phpat back into `base.neon` (#45 took it out;
  `CheckPhpatPresetTest::baseAloneDoesNotLoadPhpat()` reds if it returns) and never
  into `require` — a repository without such a rule must not install it.
...
- **Never put a `"//"` note key in `biome/base.json`.** Biome's config deserializer is
  ...
  parses); only `tests/CheckJsConfigsConsumerSmokeTest.php`, which loads the config with
  the real tool, can. Any new shipped config gets a guard that runs its actual tool, not a
  syntax check.
...
- **This repository's own `.gitattributes` is kept in step with `templates/gitattributes`
  ...
  requirement either. `tests/CheckGitattributesLockstepTest.php` (run by
  `composer ci:test:phpunit`) is its fixture-driven self-test; run against this
  repository alone the gate only ever takes the happy path. Detail and the real gap it once found (`/.build`, present in the template
...
block instead). The
own `ci.yml` adds a `js` job running the native `node:test` suite over
`bin/support/merge-config-layer.mjs` — the packaging/consumer-smoke half (packing
the package as npm ships it, installing it into a throwaway consumer, running
Biome/tsc/jscpd against the shared configs) moved to the `tests/CheckJsConfigs*Test.php`
suites (`tests/CheckJsConfigsManifestTest.php` plus the four built on
`tests/Support/AbstractJsConfigsTestCase.php`), run by the `build` job's PHPUnit step
instead — every one of those classes carries `#[Group('js-packaging')]`, so that step runs the
group on only the `php == '8.3'` floor leg (`--exclude-group js-packaging` on the
other two legs), not once per matrix leg. **The `js` job publishes the status context `JS/TS configs` — no
