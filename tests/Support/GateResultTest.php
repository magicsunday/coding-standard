 ...
 * degradedOutputProvider() are ported verbatim from the degraded() self-probe
 * loops of tests/harness.sh, removed once its last suite migrated (#71) —
 * re-derive from its final revision with
 * `git show "$(git log --diff-filter=D --format=%h -1 -- tests/harness.sh)^:tests/harness.sh" | grep -n "^for harness_degraded_probe"`.
 * The Node-related fixtures are NOT a literal port: tests/harness.sh probed two of this regex's arms
